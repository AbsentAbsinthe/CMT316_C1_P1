...
for home in dataset_file[1:]:
    ...
    vector_home_features=np.zeros(len(home_linesplit)-1)
    for i in range(len(home_linesplit)-1):
        vector_home_features[i]=float(home_linesplit[i])
...
test_dataset_file=open(train_path).readlines()
...
for home in test_dataset_file[1:]:
    ...
    vector_test_home_features=np.zeros(len(home_linesplit)-1)
    for i in range(len(home_linesplit)-1):
        vector_test_home_features[i]=float(home_linesplit[i])
